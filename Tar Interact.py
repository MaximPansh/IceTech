...
def open_datafile(path,a=1,b=200000):
    """
...
def tar_F(V, null_point=0, k=-571):
    """
...
def calculate_tar (l,r,bat,med,f_p):
    ...
    if med % 2==0:
        med=med+1
    global F_median, data_filt, data, data_mod, i, data_start, med_old, bat_old, l_old, r_old
    ...
    if i==0:
        ...
        r_old=r
        i=i+1
        data_start=open_datafile(data_file) 
        
        
    data=open_datafile(data_file,l,r)
...
def interact_point (graph_axes,f_p,l,r):
    ...
    graph_axes.clear()
    graph_axes.grid()
    graph_axes.plot(data_filt[:,0],data_filt[:,1],color='green')
    graph_axes.set_xlabel('Время, секунды')
    ...
    graph_axes.scatter(data_filt[l,0],data_filt[l,1],color='orange', s=30, marker='o') #Точка по которой строили прямую и считали D и E
    graph_axes.scatter(data_filt[r,0],data_filt[r,1],color='orange', s=30, marker='o') #Вторая точка прямой упругой зоны
