...
def onRadioButtonsClicked(label = '20 мм'):
    """
...
def onRadioButtonsClickedLayer(label = "1 слой"):
    global layer
...
#        A_p=np.trapz(y=data1[:kernel_A,0],x=(data1[:kernel_A,1] / 100)) #Определение работы разрушения
    ...
    if gran_d != 0:
        if h_ice <= 3.6 and gran_d == 3:
            graph_axes.annotate('max F=%.4g Н, w=%.4g мм' %(data[F_max,0],data[Fmax,1]), xy=(data[F_max,1],data[F_max,0]),xytext=(data[F_max,1]+.1,data[F_max,0]+.03))
        else:
            graph_axes.annotate('max F=%.4g Н, w=%.4g мм' %(data[F_max,0],data[Fmax,1]), xy=(data[F_max,1],data[F_max,0]),xytext=(data[F_max,1]+0.5,data[F_max,0]+.1))
    else:
        if  data[F_max, 0] < 1:
            graph_axes.annotate('max F=%.4g кН, w=%.4g см' %(data[F_max,0],data[Fmax,1]), xy=(data[F_max,1],data[F_max,0]),xytext=(data[F_max,1]+0.05,data[F_max,0]+.05))

        else:
            graph_axes.annotate('max F=%.4g кН, w=%.4g см' %(data[F_max,0],data[Fmax,1]), xy=(data[F_max,1],data[F_max,0]),xytext=(data[F_max,1]+0.5,data[F_max,0]+.1))
   ...
   # xytext1=(data[kernel,1]+0.25,data[kernel,0]-.2)
    ...
    if Nag==False:
        print('$r_{0}$ = %.3g м\nD = %.5g Н/м\nE = 'r'$%.4g\times10^3$ МПа' %(r1,D,E/pow(10,9))) #Выводим значения D и E
    
    #здесь \n$A_р$ = %.5g Дж во 2 позицию и ,A_p
    print('$A_{1} = %.5g$ Дж\n$A_{2} = %.5g$ Дж\n$A_{Σ} =%.5g$ Дж\n$h_л =%.4g$ cм ' %(A_p_F,A2,A_p_end,h))
    print('$k_{A1} = %.4g$\n$k_{A2} = %.4g$ \n$k_{AΣ} = %.4g$ \n$A_{2}/A_{Σ} = %.4g$ ' %(kp1,kp2,kp_sum,k_a))
